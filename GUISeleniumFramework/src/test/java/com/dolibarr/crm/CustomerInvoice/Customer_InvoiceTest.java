...
public class Customer_InvoiceTest extends BaseClass {
	...
	public void createInVoice() throws Throwable {
		...
		WebElement CustomerOption = driver.findElement(
				By.xpath("//li[contains(text(),'" + fLib.getDataFromPropertiesFile("customerName") + "')]"));
...
//    	System.out.println(InvoiceRef);
	...
	@Test
	public void createInVoiceWithValidatedStatus() throws Throwable {
		Homepage hP = new Homepage(driver);
		hP.getBillingAndPaymnets().click();
		BillingAndPayments bP = new BillingAndPayments(driver);
		bP.getCus_NewInvoiceLink().click();
		...
		WebElement product = driver
				.findElement(By.xpath("//li[contains(text(),'" + fLib.getDataFromPropertiesFile("product") + "')]"));
		product.click();
		...
		cL.getSearchRef().sendKeys(InvoiceRef);
		cL.getSearchButton().click();
		String actualInvoiceStatus = cL.getInvoiceStatus().getText();

		for (WebElement i : invoiceList) {
			if (i.equals(invoiceList)) {
				if (cL.getInvoiceStatus().equals(actualInvoiceStatus)) {
					UtilityClassObject.getTest().log(Status.PASS, "createInVoiceWithvalidatedStatus");
	...
	@Test
	public void CreateInvoiceAndPaymentAndValidateForCustomer() throws Throwable {
		Homepage hP = new Homepage(driver);
		hP.getBillingAndPaymnets().click();
		BillingAndPayments bP = new BillingAndPayments(driver);
		bP.getCus_NewInvoiceLink().click();
		...
		String InvoiceRef = cNI.getValidatedinvoiceNumb().getText();
		cNI.getYesButton().click();
		cNI.getEnterPaymentLink().click();
		cNI.getInvoiceDate().click();
		String dueAmount = cNI.getDueAmount().getText();
		cNI.getPaymentAmount().sendKeys(dueAmount);
		cNI.getPayButton().click();
		bP.getCus_listLink().click();
		Customer_List cL = new Customer_List(driver);
		List<WebElement> invoiceList = cL.getInvoceList();
		cL.getSearchRef().sendKeys(InvoiceRef);
